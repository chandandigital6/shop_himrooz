    ...
    <article class="flex flex-col group overflow-hidden rounded-md  transition-all duration-300 shadow-lg shadow-gray-200 hover:shadow-gray-500 relative h-full"
        title="{{$productName}}">
        ...
        {{--                card top section--}}
            ...
            <div class="relative shrink-0 p-4">
                ...
                @endif
                    <a href="{{route('product.show',['productId' =>$productId])}}" class="w-full h-full absolute top-0 flex flex-col right-0 pt-2.5 md:pt-3.5 px-3 md:px-4 lg:px-[18px] z-10 -mx-0.5 sm:-mx-1"
                        ...
                        <div class="relative w-full">
                            <div class="block product-count-button-position flex justify-end gap-3 ">
                                <a href="{{Auth::guard('admin')->user()? route('cart.add', ['product' => $productId]) : route('login')}}"
                                   title="Add to Cart"  class=" z-10 inline-flex items-center justify-center w-8 h-8 text-4xl rounded-full bg-[#02b290] lg:w-10 lg:h-10 text-gray-50 focus:outline-none focus-visible:outline-none">
                                ...
                                <a href="{{Auth::guard('admin')->user()?route('wishlist.add', ['product' => $productId]) : route('login')}}"
                                   title="Add to Wishlist" class="z-10 inline-flex items-center justify-center w-8 h-8 text-4xl rounded-full bg-[#02b290] lg:w-10 lg:h-10 text-gray-50 focus:outline-none focus-visible:outline-none"
                                ...
                                <a href="{{route('product.show',['productId' =>$productId])}}"
                                   title="View Details" class="z-10 inline-flex items-center justify-center w-8 h-8 text-4xl rounded-full bg-[#02b290] lg:w-10 lg:h-10 text-gray-50 focus:outline-none focus-visible:outline-none"
                                   aria-label="Count Button">
        ...
        {{--                card bottom section--}}
        <a href="{{route('product.show',['productId' =>$productId])}}" title="Click to view details">
